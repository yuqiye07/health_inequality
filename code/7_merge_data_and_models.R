...
# this script includes data merging of SCS, Gini, panel and ailment data; 
...
df_scs <- fread("df_scs.csv") # produced from 6_calculate_scs.R
...
# 9 models in table1
model1 <- fit_model("insomia",1)
model2 <- fit_model("insomia",2)
model3 <- fit_model("insomia",3)

model4 <- fit_model("anxiety_depression",1)
model5 <- fit_model("anxiety_depression",2)
model6 <- fit_model("anxiety_depression",3)

model7 <- fit_model("obesity",1)
model8 <- fit_model("obesity",2)
model9 <- fit_model("obesity",3)
...
p1 <- plot_interaction(model3,
                       title = "(a) Insomnia")
p2 <- plot_interaction(model6,
                       title = "(b) Anxiety/Depression")
p3 <- plot_interaction(model9,
                       title = "(c) Obesity")
p_legend <- plot_interaction(model9,
                             title = "(c) Obesity",
                             legend_show = T
                             )
